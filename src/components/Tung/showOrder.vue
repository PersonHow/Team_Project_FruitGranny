        ...
        showOrder() {
            fetch("http://localhost:8080/seller_Order", {
                ...
                .then(data => {
                    ...
                    console.log(this.seller_num_content)
                    this.seller_num.forEach(item =>{
                        this.check(item.order_id);
                    })
                })
                .then(function (error) {
                    console.log(error);
                ...
                <div :id="'collapse' + [index]" class="accordion-collapse collapse show" aria-labelledby="headingOne"
                    ...
                    <div class="accordion-body" v-if="elements.order_id.includes(item.num_id.substr(0, 5))">
                        ...
                        <form class="contentRadio">
                            <span id="showCondition">商品狀態：{{ item.item_condition }}</span>
                            <input type="radio" id="radioitem True" :value="true" name="option"
                                @change="chageCondition(item.num_id)">
                            <label for="itemTrue">已出貨</label>
                            <input type="radio" id="radioitem False" :value="false" name="option"
                                @change="conditionBack(item.num_id)">
